...
lint:  ## run static analysis with flake8
	python -m black --check prettycopy setup.py
	python -m flake8 prettycopy setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black prettycopy/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./prettycopy

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v prettycopy/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v prettycopy/tests --cov=prettycopy --cov-branch --cov-fail-under=75 --cov-report term-missing
