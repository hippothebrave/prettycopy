...
def test_nobullets():
    with patch("pyperclip.copy") as copy_mock, patch("pyperclip.paste") as paste_mock,\
    patch("mytest.nonewlines") as newline_mock:
...
def test_getservice():
    ...
    patch("google.oauth2.credentials.Credentials.from_authorized_user_file") as creds_file_mock, \
    patch("google_auth_oauthlib.flow.InstalledAppFlow.from_client_secrets_file") as get_secrets_mock, \
    patch("builtins.open") as open_mock, \
    patch("googleapiclient.discovery.build") as build_mock: #patch("gdocs.build") as build2_mock:
        ...
        assert build_mock.call_args.args == ('docs', 'v1',)
        ...
        assert get_secrets_mock.call_args.args == ('../credentials.json', SCOPES,)
